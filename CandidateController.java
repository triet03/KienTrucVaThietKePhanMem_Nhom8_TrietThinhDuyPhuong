...
@Controller
@RequestMapping("/admin/candidates")  // 💡 đổi prefix tránh trùng với frontend ứng viên
public class CandidateController {
    ...
    public String showUpdateForm(@PathVariable("id") long id, Model model) {
        Candidate candidate = candidateService.getCandidateById(id);
        model.addAttribute("candidate", candidate);
        return "update-candidate";  // View cho admin chỉnh ứng viên
    }

    @PostMapping("/update/{id}")  // ✅ Đã đổi route, không còn trùng
    public String updateCandidate(@PathVariable("id") long id, @ModelAttribute Candidate candidate) {
        candidateService.updateCandidate(id, candidate);
        return "redirect:/admin/candidates";
    }
